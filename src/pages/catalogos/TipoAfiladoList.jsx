// src/pages/catalogos/TipoAfiladoList.jsx
...
const TipoAfiladoList = () => {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const response = await catalogoService.getTiposAfilado();
      if (response.success) {
        setTipos(response.data);
      } else {
        setError('Error al cargar los tipos de afilado');
      }
    } catch (err) {
  ...
  const handleSaveTipo = async () => {
    ...
    try {
      ...
      if (dialogMode === 'create') {
        response = await catalogoService.createTipoAfilado(currentTipo);
      } else {
        response = await catalogoService.updateTipoAfilado(currentTipo.id, currentTipo);
      }
      ...
      if (response.success) {
        ...
        setDialogError(response.error || `Error al ${dialogMode === 'create' ? 'crear' : 'actualizar'} el tipo de afilado`);
      }
    } catch (err) {
      console.error('Error:', err);
      setDialogError(`Error al ${dialogMode === 'create' ? 'crear' : 'actualizar'} el tipo de afilado`);
    } finally {
  ...
  const handleDeleteTipo = async () => {
    ...
    setLoading(true);
    try {
      const response = await catalogoService.deleteTipoAfilado(tipoToDelete.id);
      if (response.success) {
        loadData(); // Recargar la lista
      } else {
        setError(response.error || 'Error al eliminar el tipo de afilado');
      }
    } catch (err) {
      console.error('Error:', err);
      setError('Error al eliminar el tipo de afilado');
    } finally {
      setLoading(false);
  ...
  return (
    <Box>
      ...
      <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 2 }}>
        ...
        <Typography color="text.primary">Tipos de Afilado</Typography>
      </Breadcrumbs>
        ...
        <CardContent>
          <Grid container spacing={2} alignItems="center">
            <Grid item xs={12} md={6}>
              <TextField
                fullWidth
                placeholder="Buscar tipos de afilado..."
                variant="outlined"
                size="small"
                value={searchTerm}
                    ...
                    <TableRow>
                      <TableCell colSpan={canManage ? 4 : 3} align="center" sx={{ py: 5 }}>
                        <Typography variant="body1" color="textSecondary">
                          No se encontraron tipos de afilado
                        </Typography>
                      </TableCell>
                    </TableRow>
                  )}
      ...
      {/* Diálogo para crear/editar tipos de afilado */}
      <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
        <DialogTitle>
          {dialogMode === 'create' ? 'Crear Nuevo Tipo de Afilado' : 'Editar Tipo de Afilado'}
        </DialogTitle>
        <DialogContent>
          {dialogError && (
      ...
      <Dialog open={openDeleteDialog} onClose={handleCloseDeleteDialog}>
        ...
        <DialogContent>
          <DialogContentText>
            ¿Está seguro de que desea eliminar el tipo de afilado "{tipoToDelete?.nombre}"? Esta acción no se puede deshacer.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
...
export default TipoAfiladoList;