...
const AfiladoDetail = ({ clienteFilter = false }) => {
  ...
  const [historialAfilados, setHistorialAfilados] = useState([]);
  ...
  // useEffect para cargar datos y verificar permisos
  useEffect(() => {
    const loadAfilado = async () => {
      ...
      try {
        ...
        if (afiladoResponse.success) {
          ...
          await loadRelatedData(afiladoData);
  ...
  const handleRegistrarSalida = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const afiladoResponse = await afiladoService.getAfiladoById(id);
        if (afiladoResponse.success) {
          setAfilado(afiladoResponse.data);
        }
                ...
                <Grid item xs={12}>
                  <Button
                    ...
                    onClick={() => navigate(clienteFilter ? 
                      `/mis-afilados?sierra=${afilado.sierra_id}` : 
                      `/afilados?sierra=${afilado.sierra_id}`)}
                  >
